...
type UpperTriangular<'T when 'T: struct and 'T: equality> (rowCount: int, valueAr: 'T array array) =
    do
        if valueAr.Length <> rowCount then invalidArg "valueAr" "valueAr must have the same number of arrays as there are row numbers"
        valueAr |> Array.iteri(fun i ar ->
                                if ar.Length <> rowCount - i 
                                    then invalidArg "valueAr" (sprintf("valueAr element has size inconsistent with triangular matrices")))

    member __.GetRow(index:int) : 'T array =
        let zeros = Array.zeroCreate index
        let nonzeros = Array.init (rowCount - index) (fun i -> valueAr.[i].[index])
        Array.append zeros nonzeros
    member __.GetColumn(index:int) : 'T array =
        let zeros = Array.zeroCreate (rowCount - (index+1))
        let nonzeros = Array.init (index+1) (fun i -> valueAr.[index-i].[i])
    ...
    member x.Item
        with get(index: MatrixCoordinate) : 'T = 
            ...
            else valueAr.[index.row-index.column].[index.row] 
        ...
        and set (index:MatrixCoordinate) (value: 'T) =
            ...
            valueAr.[index.row-index.column].[index.row] <- value
